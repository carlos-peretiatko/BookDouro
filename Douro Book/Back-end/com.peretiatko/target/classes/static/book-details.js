...
async function loadBookDetails(bookId) {
    try {
        const response = await fetch(`/api/livros/${bookId}`);
...
async function loadBookReviews(bookId) {
    try {
        const response = await fetch(`/api/livros/${bookId}/avaliacoes`);
...
function updateReviewsSection(reviews) {
    ...
    totalRatings.textContent = `(${reviews.length} avaliações)`;

    // Atualizar estrelas da média
    updateStarRating(document.querySelector('.ratings-summary .stars'), average);
    ...
    return `
        <div class="review-item">
            <div class="review-header">
                <span class="review-user">${review.usuario}</span>
...
async function checkIfFavorited(bookId) {
    try {
        const response = await fetch(`/api/usuarios/favoritos/${bookId}`);
